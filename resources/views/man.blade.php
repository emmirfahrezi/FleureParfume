        ...
        <div class="max-w-7xl mx-auto mt-12">
            @if (isset($products) && $products->count() > 0)
                <div id="productsGrid" class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
                    @foreach ($products as $product)
                        <a href="/detailProduk/{{ $product->id }}"
                            class="bg-white rounded-xl shadow-lg overflow-hidden transition hover:shadow-2xl block">
                            ...
                            <div class="p-4">
                                <h4 class="text-sm text-gray-500 mb-1"
                                    style="font-family: cormorant, serif !important;">
                                    {{ $product->category->name ?? 'Men' }}</h4>
                                <h3 class="text-xl font-semibold text-gray-900 truncate"
                                    style="font-family: cormorant, serif !important;">
                                    {{ $product->name }}</h3>
                                <p class="text-gray-600 mt-2" style="font-family: poppins, sans-serif !important;">Rp
                ...
                <div id="productsList" class="hidden space-y-4">
                    @foreach ($products as $product)
                        <a href="/detailProduk/{{ $product->id }}"
                            class="bg-white rounded-xl shadow-lg overflow-hidden transition hover:shadow-2xl flex flex-col sm:flex-row group block">

            ...
            @if (isset($products) && $products->count() > 0)
                <div id="productsGrid" class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
                    @foreach ($products as $product)
                        <a href="/detailProduk/{{ $product->id }}" class="bg-white rounded-xl shadow-lg overflow-hidden transition hover:shadow-2xl block">
                            <div class="relative">
                                @if ($product->image)
                            ...
                            <div class="p-5 space-y-2">
                                ...
                                <p class="text-sm text-gray-500" style="font-family: poppins, sans-serif;">Aroma pilihan
                                    untuk segala suasana.</p>
                            </div>
                        </a>
                    @endforeach
                </div>
                ...
                <div id="productsList" class="hidden space-y-4">
                    @foreach ($products as $product)
                        <a href="/detailProduk/{{ $product->id }}"
                            class="bg-white rounded-xl shadow-lg overflow-hidden flex flex-col sm:flex-row transition hover:shadow-2xl block">
                            @if ($product->image)
                                <img src="{{ asset('storage/' . $product->image) }}" alt="{{ $product->name }}"
                            ...
                            <div class="p-4 sm:p-6 flex flex-col justify-center gap-2 flex-grow">
                                <div class="text-xs sm:text-sm text-gray-500" style="font-family: poppins, sans-serif;">
                                ...
                                <p class="text-base sm:text-lg text-gray-700"
                                    ...
                                    {{ number_format($product->price, 0, ',', '.') }}</p>
                                <p class="text-sm text-gray-500" style="font-family: poppins, sans-serif;">Aroma
                                    pilihan untuk segala suasana.</p>
                            </div>
                        </a>
                    @endforeach
                </div>

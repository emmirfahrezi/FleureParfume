<x-layoutCategories>
    ...
    <div class="relative isolate px-6 pt-20 lg:px-20 min-h-[420px]" style="background: linear-gradient(135deg, #f4ede1 0%, #e9dcc3 45%, #f7f1e7 100%);">
        ...
        <div class="relative max-w-5xl w-full py-8 sm:py-12 lg:py-16 flex flex-col gap-5 text-gray-900">
            ...
            <div class="space-y-4">
                ...
                <h1 class="text-5xl sm:text-6xl lg:text-7xl font-light leading-tight" style="font-family: cormorant, serif !important;">
    ...
    <div class="relative isolate px-6 pt-14 lg:px-20 min-h-screen py-10">
        ...
        <div class="max-w-7xl mx-auto mt-12">
                ...
                @foreach ($bestSellers as $product)
                    <div class="bg-white rounded-xl shadow-lg overflow-hidden transition hover:shadow-2xl">
                        <div class="relative">
                            ...
                            <span class="absolute top-3 left-3 px-3 py-1 rounded-full text-xs uppercase tracking-wide bg-white/90 text-gray-900" style="font-family: poppins, sans-serif;">
                                {{ $product['category'] }}
                            </span>
                        </div>
                        <div class="p-5 space-y-2">
                            <h3 class="text-2xl font-semibold text-gray-900" style="font-family: cormorant, serif !important;">{{ $product['name'] }}</h3>
                            <p class="text-gray-700 text-lg" style="font-family: poppins, sans-serif !important;">Rp {{ number_format($product['price'], 0, ',', '.') }}</p>
                            <p class="text-sm text-gray-500" style="font-family: poppins, sans-serif;">Aroma pilihan untuk segala suasana.</p>
                        </div>
                    </div>
                @endforeach
            </div>
            ...
            <div id="productsList" class="hidden space-y-4">
                @foreach ($bestSellers as $product)
                    <div class="bg-white rounded-xl shadow-lg overflow-hidden flex flex-col sm:flex-row transition hover:shadow-2xl">
                        <img src="{{ asset($product['image']) }}" alt="{{ $product['name'] }}" class="w-full sm:w-48 h-48 object-cover flex-shrink-0">
                        <div class="p-4 sm:p-6 flex flex-col justify-center gap-2 flex-grow">
                            <div class="text-xs sm:text-sm text-gray-500" style="font-family: poppins, sans-serif;">{{ $product['category'] }}</div>
                            <h3 class="text-2xl font-semibold text-gray-900" style="font-family: cormorant, serif !important;">{{ $product['name'] }}</h3>
                            <p class="text-base sm:text-lg text-gray-700" style="font-family: poppins, sans-serif !important;">Rp {{ number_format($product['price'], 0, ',', '.') }}</p>
                            <p class="text-sm text-gray-500" style="font-family: poppins, sans-serif;">Aroma pilihan untuk segala suasana.</p>
                        </div>
                    </div>
                @endforeach
    ...
    <div id="filterSidebar" class="fixed top-0 left-0 h-full w-[360px] bg-white -translate-x-full transition-transform duration-300 ease-in-out z-50 overflow-y-auto">
        ...
        <div class="px-6 mt-10">
            ...
            <div class="relative h-2 bg-gray-200 rounded-full mb-6">
                ...
                <input id="priceMinRange" type="range" min="0" max="500" value="199" step="1" class="absolute inset-0 w-full h-6 opacity-0 cursor-pointer z-30">
                <input id="priceMaxRange" type="range" min="0" max="500" value="375" step="1" class="absolute inset-0 w-full h-6 opacity-0 cursor-pointer z-20">
            </div>

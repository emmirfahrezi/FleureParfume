            ...
            @if (isset($products) && $products->count() > 0)
                ...
                <div id="productsGrid" class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
                    @foreach ($products as $product)
                        <a href="/detailProduk/{{ $product->id }}" class="bg-white rounded-xl shadow-lg overflow-hidden transition hover:shadow-2xl block border border-gray-100">
                            <div class="relative">
                                @if ($product->image)
                                ...
                                @endif
                                <span
                                    class="absolute top-3 left-3 px-3 py-1 rounded-full text-xs uppercase tracking-wide bg-white/90 text-gray-900"
                                    style="font-family: poppins, sans-serif;">
                    ...
                    @foreach ($products as $product)
                        <a href="/detailProduk/{{ $product->id }}"
                            ...
                            <div class="p-4 sm:p-6 flex flex-col justify-center gap-2 flex-grow">
                                <div class="text-xs sm:text-sm text-gray-500">{{ $product->category->name ?? 'General' }}</div>
                                <h3 class="text-2xl font-semibold text-gray-900"
                                    style="font-family: cormorant, serif !important;">{{ $product->name }}</h3>
    ...
    <div id="filterSidebar"
        ...
        <form action="{{ request()->url() }}" method="GET">
            ...
            <div class="px-6 mt-10">
                ...
                <div class="flex gap-4">
                    <div class="border px-4 py-2 text-sm" id="priceMinLabel">Rp {{ number_format(request('min_price', 0) * 1000, 0, ',', '.') }}</div>
                    <div class="border px-4 py-2 text-sm" id="priceMaxLabel">Rp {{ number_format(request('max_price', 500) * 1000, 0, ',', '.') }}</div>
                </div>
                <button type="submit" class="w-full mt-4 bg-black text-white py-2 rounded text-sm hover:bg-gray-800 transition">Apply Price Filter</button>
            </div>
        </form>
        ...
        <div class="px-6 mt-14 mb-10">
            ...
            <ul class="space-y-4 font-semibold text-lg">
                ...
                <li class="pt-4">
                    <a href="{{ request()->url() }}" class="text-red-500 text-sm font-normal underline">Clear All Filters</a>
                </li>
            </ul>
        </div>
    </div>
